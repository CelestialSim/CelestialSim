...
2. [`CesDivConstraintShader`](xref:rng_gdot.addons.celestial_sim.client.division.CesDivConstraintShader) validates the triangles flagged for subdivision and clears any that would introduce more than a 1-level jump with neighbors.
3. [`CesDivShader`](xref:rng_gdot.addons.celestial_sim.client.division.CesDivShader) performs the actual 1→4 split, creating midpoint vertices, updating neighbor relationships, and maintaining the hierarchy used by [`CesStateVerifier`](xref:ces.Rendering.CesStateVerifier).
...
5. **Layer Processing** – Optional layers like [`CesNormalizePos`](xref:rng_gdot.scripts.client.Layers.CesNormalizePos) or water simulations modify positions before rendering.
