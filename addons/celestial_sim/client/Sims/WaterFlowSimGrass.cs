...
[Tool]
public partial class WaterFlowSimGrass: CesSimLayer
{
    private readonly int Seed = new Random().Next();

    [Export] public bool goDown = false;
    
    [Export] public float grassSpreadSpeed = 0.1f;
    
    [Export] public float waterBecomesGrass = 0.1f;
    ...
    )
    {
        ...
        for (var i = 0; i < numTriangles; i++)
        {
            if (!tDivided[i])
            {
                ...
                if (minIdx < 3 && oldSimValue[i] > waterBecomesGrass)
                {
                ...
                float neighWater = 0;
                neighWater += (float)oldSimValue[(int)tNeightAb[i]];
                neighWater += (float)oldSimValue[(int)tNeightBc[i]];
                neighWater += (float)oldSimValue[(int)tNeightCa[i]];
                
                if (simValue[i] <= waterBecomesGrass && neighWater > 0)
                {
                    if (neighWater > waterBecomesGrass * 3.0f)
                        neighWater = waterBecomesGrass * 3.0f;
                    simValue[i] = (neighWater / 3.0f) * grassSpreadSpeed;
                }
            }
            else
            {
