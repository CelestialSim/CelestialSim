...
Example single-shot wrapper pattern (this repo has no scenes of its own — the
runnable demo/benchmark scenes live in the sibling **CelestialSimDemo** repo,
e.g. `scenes/celestial_v4_hq.tscn`, and are run from there; point the wrapper's
`PackedScene` at your own target scene):

```tscn
[gd_scene load_steps=3 format=3]

[ext_resource type="PackedScene" path="res://scenes/your_target_scene.tscn" id="1_scene"]
